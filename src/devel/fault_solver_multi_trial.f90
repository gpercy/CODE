...
!  real(kind=CUSTOM_REAL), intent(inout) :: T0(3,:)
...
!  integer            :: fault_nodes,inode
!  real(kind=CUSTOM_REAL)   :: Ti0(3,:)
!  real(kind=CUSTOM_REAL)   :: imus(:),imud(:),iDc(:),iDr(:)
!  integer                  :: icoord(3,:)
  
!  write(filepar,"('DATA/FAULT/Par_init_faults_par',I0)") iflt
...
!   theta : slip
   ...
   fi%theta = max(f%theta,(fi%idrop+1)*f%dr)

   where (( fi%theta == f%theta ) .and. ( fi%idrop+1 < NDROPS) )
     fi%idrop  = fi%idrop + 1
     fi%mus    = fi%mud
     fi%mud    = f%mud - fi%idrop*(u_drop)      
   end where
       
   mu    = fi%mus -((fi%mus-fi%mud)/f%dc) *(f%theta-fi%idrop*f%dr)
   mu    = max(mu,fi%mud)
