...
module fault_solver
 ...
 !Type of input for initial fault Parameters 
  ...
  logical, save :: SIMULATION_TYPE_DYN = .false.,SCEC_OUTPUT=.false.
...
! WARNING: if V_HEALING is negative we turn off healing
  ...
  bc%MU = swf_mu(bc%swf)
  if (.not.OPT) call init_dataT(bc%dataT,bc%coord,bc%nglob,NT,iflt)
  if (OPT) call init_dataT(bc%dataT,bc%coord,bc%nglob,NTOUT,iflt)
...
! write dataT every NTOUT time step or at the end of simulation
  if ( mod(it,NTOUT) == 0 .or. it==NSTEP) then 
   if (SCEC_OUTPUT) then
       ...
       call write_dataT_output(bc%dataT) 
   endif
  endif
...
! OUTPUTS
 ...
 subroutine init_dataT(DataT,coord,nglob,NT,iflt)
  ! NT = Number of time steps to write data on (NTOUT)
...
!---------------------------------------------------------------
  ...
  iter=mod(itime,NTOUT)
  if (iter==0) iter=NTOUT

  do i=1,dataT%npoin
    k = dataT%iglob(i)
    dataT%d1(iter,i) = d(1,k)
    dataT%d2(iter,i) = d(2,k)
    dataT%v1(iter,i) = v(1,k)
    dataT%v2(iter,i) = v(2,k)
    dataT%t1(iter,i) = t(1,k)
    dataT%t2(iter,i) = t(2,k)
    dataT%t3(iter,i) = t(3,k)
  enddo
...
!-----------------------------------------------------------------
  ...
  do i = 1,size(faults)
   if (SCEC_OUTPUT) then 
        ...
        call write_dataT_output(faults(i)%dataT)
   endif
  enddo
...
!------------------------------------------------------------------------
 subroutine write_dataT_output(dataT)
 
 type(dataT_type), intent(in) :: dataT
! real(kind=CUSTOM_REAL), intent(in) :: itime
 
 integer :: IOUT=121  
 open(IOUT,file='OUTPUT_FILES/FAULT_STATIONS.dat',status='replace')
 write(IOUT) dataT%npoin !Number of stations
 do i=1,dataT%npoin
   write(IOUT) dataT%d1(:,i),dataT%v1(:,i),dataT%t1(:,i),&
               dataT%d2(:,i),dataT%v2(k,i),dataT%t2(:,i),&
               dataT%t3(:,i)
 enddo
 close(IOUT) 

 end subroutine write_dataT_output 
