...
module fault_solver
 ...
 !Number of time steps defined by the user : NTOUT
  integer, save                :: NTOUT,NSNAP,NT0,NLEN
 !Type of input for initial fault Parameters 
  ...
  logical, save :: SIMULATION_TYPE_DYN = .false.,DISP_FAULT=.false.
...
! WARNING: if V_HEALING is negative we turn off healing
  ...
  if (DISP_FAULT) call init_dataT0(bc%dataT0,bc%coord,bc%nglob,iflt)
...
!-- intermediate storage of outputs --
  ...
  if (mod(it,NT0)==0) call store_dataT0(bc%dataT0,bc%D,bc%V,bc%T,it)

!-- outputs --
! write dataT every NTOUT time step or at the end of simulation
  if ( mod(it,NTOUT) == 0 .or. it==NSTEP) then 
   call SCEC_write_dataT(bc%dataT,bc%dt,it)
   call write_dataT0(bc%dataT0,bc%dt,it,iflt)
  endif
...
!---------------------------------------------------------------
 subroutine init_dataT0(dataT0,coord,nglob,ifault)
 
  integer,intent(in) :: ifault,nglob
  real(kind=CUSTOM_REAL), intent(in) :: coord(3,nglob)
  type (dataT_type), intent(out)     :: DataT0
   
  integer :: IIN=215,IIF=216,ierr=0,ier=0,i
  character(len=50) filename
  character(len=256) filename_fault,LOCAL_PATH,clean_LOCAL_PATH,final_LOCAL_PATH
  character(len=100) string_read
  character(len=10) :: name 
  integer, allocatable, dimension(:,:) :: el_fault
  real(kind=CUSTOM_REAL), allocatable, dimension(:,:) :: nodes_coord,nodes_fault,fault_nodes_coord
  real(kind=CUSTOM_REAL) :: dummy_val=1000000e0_CUSTOM_REAL,distkeep,dist
  integer :: nel_up,nel_down,nnodes,nel_fault,dummy
  integer :: nfault_nodes,ifault_nodes,inode,iglob,iel
  character(len=256),dimension(7) :: filenames
  character(len=2),dimension(1:7) :: NAM=(/'D1','V1','T1','D2','V2','T2','T3'/)
  integer,dimension(1:7) :: IOUTA=(/201,202,203,204,205,206,207/)
   
  name='LOCAL_PATH'  
 ! opens file DATA/Par_file . Note : Here c-file called param_reader.c is used 
  filename = 'DATA/Par_file'
  call param_open(filename, len(filename), ierr);
  if (ierr /= 0) stop 'error opening Par_file'

 ! Reading LOCAL_PATH from Par_file
  call param_read(string_read, len(string_read), name, len(name), ierr);
  if (ierr /= 0) stop 'error finding LOCAL_PATH'
  LOCAL_PATH = string_read
 
 ! fault file name per fault . 
  write(filename_fault,"('fault_file_',I0,'.dat')") ifault
  
  !supress white spaces if any
  clean_LOCAL_PATH = adjustl(LOCAL_PATH)
 
  ! create full final local path 
  final_LOCAL_PATH = clean_LOCAL_PATH(1:len_trim(clean_LOCAL_PATH))//'/'
  
  open(unit=IIF,file=final_LOCAL_PATH(1:len_trim(final_LOCAL_PATH))//&
                     filename_fault(1:len_trim(filename_fault)),status='old'&
                     ,action='read',iostat=ier)
  if( ier /= 0 ) stop 'error opening fault_file_xxx file'

  open(unit=IIN,file=final_LOCAL_PATH(1:len_trim(final_LOCAL_PATH))//&
                     'nodes_coords_file',status='old'&
                     ,action='read',iostat=ier)
  if( ier /= 0 ) stop 'error opening fault_file_xxx file' 


 !Reading fault_file_xxxx and nodes_coord_file
 read(IIF,*)  nel_up,nel_down
 read(IIN,*)  nnodes
 if (nel_up /= nel_down) stop 'number of fault nodes do not conside'
 nel_fault=nel_up 
 allocate(el_fault(4,nel_fault))
 allocate(nodes_coord(3,nnodes))
 allocate(nodes_fault(3,nnodes))

 ! loads fault nodes per fault element :(Warning double fault nodes taken here)
 do i=1,nel_fault
  read(IIF,*) dummy,el_fault(:,i) 
 enddo
 ! loading nodes coordinates 
 do i=1,nnodes
  read(IIN,*) dummy,nodes_coord(:,i)
 enddo
 ! cloasing files nodes_coords_file and fault_file_xxx 
 close(IIN)
 close(IIF)
 
! Filling nodes_fault with dummy value, this is used to remove double fault nodes
 nodes_fault(3,:)=dummy_val ! Initial value Z = 1000km. 
! loading all fault nodes and allocating double nodes with the same index
 do iel=1,nel_fault
    do inode=1,4
      ifault_nodes=el_fault(inode,iel)
      nodes_fault(1,ifault_nodes)=nodes_coord(1,ifault_nodes)
      nodes_fault(2,ifault_nodes)=nodes_coord(2,ifault_nodes)
      nodes_fault(3,ifault_nodes)=nodes_coord(3,ifault_nodes)
    enddo
 enddo
...
! Counting unique nodes. 
 nfault_nodes=0
 do inode=1,nnodes 
        if (nodes_fault(3,inode)/= dummy_val) then
            nfault_nodes=nfault_nodes+1
        endif
 enddo
 allocate(fault_nodes_coord(3,nfault_nodes))

! allocating unique fault nodes. 
 ifault_nodes=0 
 do inode=1,nnodes
        if (nodes_fault(3,inode)/=dummy_val) then
            ifault_nodes=ifault_nodes+1
            fault_nodes_coord(1,ifault_nodes)=nodes_fault(1,inode)
            fault_nodes_coord(2,ifault_nodes)=nodes_fault(2,inode)
            fault_nodes_coord(3,ifault_nodes)=nodes_fault(3,inode)
        endif
 enddo 
 
 !search nearest node
 DataT0%npoin=nfault_nodes
 allocate(DataT0%iglob(DataT0%npoin))
 do ifault_nodes=1,nfault_nodes
    distkeep = huge(distkeep)
    do iglob=1,nglob
      dist = sqrt((coord(1,iglob)-fault_nodes_coord(1,ifault_nodes))**2 &
                + (coord(2,iglob)-fault_nodes_coord(2,ifault_nodes))**2 &
                + (coord(3,iglob)-fault_nodes_coord(3,ifault_nodes))**2)  
      if (dist < distkeep) then
        distkeep = dist
        DataT0%iglob(ifault_nodes) = iglob   
      endif 
    enddo
 enddo  
           
! testing this module with an output file
  open(IIN,file='OUTPUT_FILES/Fault_points.XYZ.dat',status='replace',action='write')
   write(IIN,*) size(fault_nodes_coord,2)
   write(IIN,*) fault_nodes_coord(1,:),fault_nodes_coord(2,:),fault_nodes_coord(3,:)
  close(IIN) 

! Outputs of fault parameters : slip,slip_rate,....
  do i=1,7
    write(filenames(i),"('OUTPUT_FILES/Fault_points_F',I0,'.',A,'.bin')") ifault,NAM(i)
    open(IOUTA(i),file=trim(filenames(i)),form='unformatted',action='write',status='replace')
      write(IOUTA(i)) DataT0%npoin
      write(IOUTA(i)) fault_nodes_coord(1,:)
      write(IOUTA(i)) fault_nodes_coord(2,:)
      write(IOUTA(i)) fault_nodes_coord(3,:)
    close(IOUTA(i))
  enddo 

!  3. allocate arrays and set to zero
!  NT0 time step taken to be saved in the output file
  NLEN = int(NTOUT/NT0)
  allocate(DataT0%d1(NLEN,DataT0%npoin))
  allocate(DataT0%v1(NLEN,DataT0%npoin))
  allocate(DataT0%t1(NLEN,DataT0%npoin))
  allocate(DataT0%d2(NLEN,DataT0%npoin))
  allocate(DataT0%v2(NLEN,DataT0%npoin))
  allocate(DataT0%t2(NLEN,DataT0%npoin))
  allocate(DataT0%t3(NLEN,DataT0%npoin))
  DataT0%d1 = 0e0_CUSTOM_REAL
  DataT0%v1 = 0e0_CUSTOM_REAL
  DataT0%t1 = 0e0_CUSTOM_REAL
  DataT0%d2 = 0e0_CUSTOM_REAL
  DataT0%v2 = 0e0_CUSTOM_REAL
  DataT0%t2 = 0e0_CUSTOM_REAL
  DataT0%t3 = 0e0_CUSTOM_REAL
...
!---------------------------------------------------------------
  subroutine store_dataT0(dataT0,d,v,t,itime)

  type(dataT_type), intent(inout) :: dataT0
  real(kind=CUSTOM_REAL), dimension(:,:), intent(in) :: d,v,t
  integer, intent(in) :: itime
 
  integer :: i,k,isave,itime0
 
  itime0 = int(itime/NT0)
  isave  = mod(itime0,NLEN)
  if (isave==0) isave=NLEN

  do i=1,dataT0%npoin
      k = dataT0%iglob(i)
      dataT0%d1(isave,i) = d(1,k)
      dataT0%d2(isave,i) = d(2,k)
      dataT0%v1(isave,i) = v(1,k)
      dataT0%v2(isave,i) = v(2,k)
      dataT0%t1(isave,i) = t(1,k)
      dataT0%t2(isave,i) = t(2,k)
      dataT0%t3(isave,i) = t(3,k)
  enddo

  end subroutine store_dataT0
...
!------------------------------------------------------------------------
  subroutine write_dataT0(dataT0,DT,itime,iflt)
   
  type(dataT_type), intent(in) :: dataT0
  integer,intent(in) :: itime,iflt
  real(kind=CUSTOM_REAL),intent(in) :: DT 

  real(kind=CUSTOM_REAL) :: t0,time 
  character(len=256),dimension(7) :: filename
  character(len=2),dimension(1:7) :: NAM=(/'D1','V1','T1','D2','V2','T2','T3'/)
  integer :: i,k,ier
  integer,dimension(1:7) :: IOUT=(/201,202,203,204,205,206,207/)
 
  t0=((int(itime/NTOUT)-1)*NTOUT)*DT ! prior time iteration block "NTOUT"
  do i=1,7
    write(filename(i),"('OUTPUT_FILES/Fault_points_F',I0,'.',A,'.bin')") iflt,NAM(i)
    open(IOUT(i),file=trim(filename(i)),form='unformatted',action='write',position='append',status='old',iostat=ier)
    if (ier/=0) stop 'Fault parameter file: slip, slip_rate, ... has not been found'
  enddo 

  do k=1,NLEN
    time=t0+k*NT0*DT
    write(IOUT(1)) time,dataT0%d1(k,:)
    write(IOUT(2)) time,dataT0%v1(k,:)
    write(IOUT(3)) time,dataT0%t1(k,:)/1.0e6_CUSTOM_REAL
    write(IOUT(4)) time,dataT0%d2(k,:)
    write(IOUT(5)) time,dataT0%v2(k,:)
    write(IOUT(6)) time,dataT0%t2(k,:)/1.0e6_CUSTOM_REAL
    write(IOUT(7)) time,dataT0%t3(k,:)/1.0e6_CUSTOM_REAL
  enddo
  do i=1,7
     close(IOUT(i))
  enddo
  end subroutine write_dataT0
!------------------------------------------------------------------------
